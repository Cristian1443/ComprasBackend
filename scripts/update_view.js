...
await client.query(`DROP VIEW IF EXISTS v_solicitudes_resumen CASCADE`);
await client.query(`
CREATE VIEW v_solicitudes_resumen AS
 SELECT s.id,
    ...
    u_sup_cont.cargo AS supervision_cargo,
    ...
    ( SELECT count(*) AS count
           FROM documentos d
          WHERE (d.solicitud_id = s.id)) AS num_documentos,
    ( SELECT count(*) AS count
           FROM comentarios c
          WHERE (c.solicitud_id = s.id)) AS num_comentarios,
    s.titulo_contrato
   FROM ((((((solicitudes s
...
`);
console.log('✓ Vista v_solicitudes_resumen actualizada con titulo_contrato');
client.release();
await pool.end();
